...
class CLIDashboard {
        ...
        configurator
          ...
          .then(() => {
            this.config = JSON.parse(
              fs.readFileSync(__dirname + "/../goldshell.json", "utf8")
            );
            this.startMiner();
            this.initBlessed();
          })
  ...
  updateAsicStats(data) {
    ...
    this.asics[key].data.temperature.push({
      temperature: parseFloat(data.data.temp),
      time: moment().format("HH:mm"),
    });
    this.asics[key].data.fan.push({
      fans: parseFloat(data.data.fanRpm),
      time: moment().format("HH:mm"),
    });
    this.asics[key].data.asicCoreClock.push({
      clock: parseFloat(data.data.frequency),
      time: moment().format("HH:mm"),
    });
    this.asics[key].data.asicMemoryClock.push({
      clock: parseFloat(data.data.frequency),
      time: moment().format("HH:mm"),
    });
    ...
    this.asics[key].data.power.push({
      power: powerW,
      time: moment().format("HH:mm"),
    });
    this.asics[key].data.voltage.push({
      voltage: parseFloat(data.data.voltage),
      time: moment().format("HH:mm"),
    });
  ...
  updateSparkLines() {
    ...
    Object.keys(this.asics).map((k) => {
      ...
      if (mData.length > this.linechartTicksMax) {
        mData = mData.slice(
          mData.length - this.linechartTicksMax / 2,
          mData.length
        );
    ...
    Object.keys(this.asics).map((k) => {
      let asic = this.asics[k];
      let name =
        asic.info.serialPort.replace("/dev/", "") + "." + asic.info.modelName;
      let data = [];
      let last = 0;
  ...
  updateFanChart() {
    ...
    Object.keys(this.asics).map((k, i) => {
      ...
      if (asic.data.power.length > this.linechartTicksMax) {
        this.asics[k].data.power = this.asics[k].data.power.slice(
          this.asics[k].data.power.length - this.linechartTicksMax / 2,
          this.asics[k].data.power.length
        );
      }
      if (asic.data.fan.length > this.linechartTicksMax) {
        this.asics[k].data.fan = this.asics[k].data.fan.slice(
          this.asics[k].data.fan.length - this.linechartTicksMax / 2,
          this.asics[k].data.fan.length
        );
      }
      if (i == 0) {
        eV = asic.data.power.map((d) => {
          return d.power;
        });
    ...
    this.energyArea.setData(
      [
        "Energy \x1b[36m" + eVLast + "W\x1b[0m",
        "Avg Fan Speed \x1b[36m" + Math.floor(fVLast) + "\x1b[0m",
      ],
      [eV, fV]
    );
  }
  updatePowerChart() {
    ...
    Object.keys(this.asics).map((asicKey) => {
      ...
      let name =
        asicData.info.serialPort.replace("/dev/", "") +
        "." +
        asicData.info.modelName;
      ...
      if (tempData.length > this.linechartTicksMax) {
        tempData = tempData.slice(
          tempData.length - this.linechartTicksMax / 2,
          tempData.length
        );
  ...
  startMiner() {
    ...
    let minerProcess = spawn("node", [__dirname + "/../mine.js"], {
      env: process.env,
    });
        ...
        try {
          ...
          if (json.type == "registration") {
            ...
            if (
              ...
              typeof json.granule != "undefined" /*&& !didConfirm*/
            ) {
  ...
  initFanfare(json) {
    ...
    logInitLines = logInitLines.map((line) => {
      ...
      if (this.logsBox.logLines.length > 2000) {
        this.logsBox.logLines = this.logsBox.logLines.slice(
          this.logsBox.logLines.length - 500,
          this.logsBox.logLines.length
        );
      }
    });
  }
    ...
    jsonLines.map((json) => {
      switch (json.type) {
        ...
        case "asicStats":
          ...
          this.asics[platform].data.hashrate.push({
            hashrate: hr,
            time: moment().format("HH:mm"),
          });
          this.asics[platform].data.hashrate120.push({
            hashrate: hr120,
            time: moment().format("HH:mm"),
          });
          this.actuallyLog(
            "ASIC " +
              ...
              modelName +
              ": \x1b[36m" +
              numeral(hr).format("0.00") +
              "GH AVG: " +
              numeral(hr120).format("0.00") +
              "GH\x1b[0m"
          );
          this.drawHashrate();
        ...
        case "error":
          ...
          if (isDisconnect) {
            this.actuallyLog(
              "\x1b[31mASIC " + json.data.asicID + " DISCONNECTED\x1b[0m"
            );
          }
          break;
      }
  ...
  drawHashrate() {
    ...
    Object.keys(this.asics).map((asicKey) => {
      ...
      if (hashData.length > this.linechartTicksMax) {
        hashData = hashData.slice(
          hashData.length - this.linechartTicksMax / 2,
          hashData.length
        );
      ...
      if (hd120.length > this.linechartTicksMax) {
        hd120 = hd120.slice(
          hd120.length - this.linechartTicksMax / 2,
          hd120.length
        );
    ...
    Object.keys(this.asics).map((key) => {
      ...
      sum120 +=
        asic.data.hashrate120[asic.data.hashrate120.length - 1].hashrate;
  ...
  updateStats(data, type) {
    ...
    let statsData = [
      "Valid Shares: \x1b[36m" +
        numeral(this.statsData["shares"]).format("0a") +
        "\x1b[0m",
      "Invalid Shares: \x1b[36m" +
        numeral(this.statsData["errors"]).format("0a") +
        "\x1b[0m",
      "Last Share: \x1b[36m" +
        (typeof this.statsData["last"] == "undefined"
          ? "none"
          : this.statsData["last"].format("MMM-DD HH:mm")) +
        "\x1b[0m",
      "Started: \x1b[36m" +
        this.statsData["started"].format("MMM-DD HH:mm") +
        "\x1b[0m",
      "Rig Hashrate: \x1b[36m" +
        (numeral(this.statsData["hashrate"]).format("0.00") + "GH") +
        "\x1b[0m",
      "Avg Hashrate: \x1b[36m" +
        (numeral(this.statsData["hashrate120"]).format("0.00") + "GH") +
        "\x1b[0m",
      "Target: \x1b[36m0x" + this.statsData.target.slice(0, 32) + "\x1b[0m",
      "Share Diff: \x1b[36m" +
        numeral(this.statsData.difficulty).format("0.0a") +
        "\x1b[0m",
      "Network Diff: \x1b[36m" +
        numeral(this.statsData.networkDifficulty).format("0.0a").toUpperCase() +
        "\x1b[0m",
    ];
  ...
  addASICs(asicList) {
    ...
    let asicItems = asicList.map((asic, asicI) => {
      ...
      let color_8bit = [
        Math.floor(colorBytes.r),
        Math.floor(colorBytes.g),
        Math.floor(colorBytes.b),
      ]; //(red << 5) | (green << 2) | blue;
    ...
    let asicSpeedPosX = 0;
    let asicSpeedArea = this.grid.set(
      2,
      asicSpeedPosX,
      2,
      1,
    ...
    energyArea.setData(
      ["Energy \x1b[36m---W\x1b[0m", "Fan Speed \x1b[36m--%\x1b[0m"],
      [eV, fV]
    );
    let statsData = [
