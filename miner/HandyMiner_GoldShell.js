...
class HandyMiner {
    ...
    if (typeof process.argv[2] != "undefined") {
      if (process.argv[2].indexOf(".json") >= 0) {
        ...
        if (!process.env.HANDYRAW) {
          console.log(
            "\x1b[36m#### LOADING CONFIG FILE: \x1b[0m" + configFileName
          );
        }
      }
    }
      ...
      if (this.config.mode == "pool") {
        ...
        if (typeof this.config.poolDifficulty != "undefined") {
          ...
          if (
            ...
            this.config.host.indexOf("6block") >= 0
          ) {
            ...
            if (process.env.HANDYRAW) {
              process.stdout.write(
                JSON.stringify({
                  type: "stratumLog",
                  data: "setting 6block pool min difficulty at 2048",
                }) + "\n"
              );
            } else {
              console.log(
                "\x1b[36m6BLOCK POOL MINIMUM DIFF SET AT 2048\x1b[0m"
              );
            }
          } else if (
            this.poolDifficulty < 512 &&
            this.poolDifficulty >= 0 &&
            this.config.host.indexOf("hnspool") >= 0
          ) {
            this.poolDifficulty = 512;
            ...
            if (process.env.HANDYRAW) {
              process.stdout.write(
                JSON.stringify({
                  type: "stratumLog",
                  data: "setting hnspool pool min difficulty at 512",
                }) + "\n"
              );
            } else {
              console.log(
                "\x1b[36mHNSPOOL POOL MINIMUM DIFF SET AT 512\x1b[0m"
              );
            }
          }
    ...
    */
    ...
    if (process.env.HANDYRAW) {
      process.stdout.write(
        JSON.stringify({
          type: "stratumLog",
          data: "stratum will try to connect " + this.host + ":" + this.port,
        }) + "\n"
      );
    } else {
      console.log(
        "\x1b[36mstratum will try to connect \x1b[0m" +
          this.host +
          ":" +
          this.port
      );
    ...
    if (!fs.existsSync(process.env.HOME + "/.HandyMiner/version.txt")) {
      ...
      fs.writeFileSync(
        process.env.HOME + "/.HandyMiner/version.txt",
        myMin.toString()
      );
    }
    let gpus = this.gpuListString.split(",").map((s) => {
      return s.trim();
    });
    let platform = this.platformID;
    gpus.map((gpuID) => {
      fs.writeFileSync(
        process.env.HOME + "/.HandyMiner/" + platform + "_" + gpuID + ".work",
        ""
      );
    });
  ...
  startSocket() {
      ...
      (socket) => {
        ...
        if (process.env.HANDYRAW) {
          process.stdout.write(
            JSON.stringify({
              type: "stratumLog",
              data: "stratum connected to " + this.host + ":" + this.port,
            }) + "\n"
          );
        } else {
          console.log(
            "\x1b[36mstratum server is connected to\x1b[0m " +
              this.host +
              ":" +
              this.port
          );
        ...
        if (
          this.config.mode == "pool" &&
          this.host.toLowerCase().indexOf("hnspool") >= 0
        ) {
          ...
          this.server.write(
            JSON.stringify({
              id: this.targetID,
              method: "authorize",
              params: [stratumUser, stratumPass, "handy-miner-v0.0.0"],
            }) + "\n"
          );
          ...
          if (this.config.mode == "solo") {
            this.server.write(
              JSON.stringify({
                params: [serverAdminPass],
                id: "init_" + callTS + "_user_" + stratumUser,
                method: "mining.authorize_admin",
              }) + "\n"
            );
          }
          if (this.host.toLowerCase().indexOf("poolflare") == -1) {
            this.server.write(
              JSON.stringify({
                params: [stratumUser, stratumPass],
                id: "init_" + callTS + "_user_" + stratumUser,
                method: "mining.add_user",
              }) + "\n"
            );
          }
          this.server.write(
    ...
    this.server.on("error", (response) => {
      if (
        ...
        response.syscall == "connect" &&
        !this.isKilling
      ) {
        if (process.env.HANDYRAW) {
          process.stdout.write(
            '{"type":"error","message":"STRATUM CONNECTION REFUSED, TRYING AGAIN IN 20s"}\n'
          );
        } else {
          console.log(
            "HANDY:: \x1b[36mSTRATUM CONNECTION REFUSED, TRYING AGAIN IN 20s\x1b[0m"
          );
        }
    ...
    if (!this.isKilling && !this.hasConnectionError) {
      ...
      if (process.env.HANDYRAW) {
        process.stdout.write(
          JSON.stringify({
            type: "error",
            message: "STRATUM CONNECTION WAS CLOSED. RECONNECTING NOW.",
          }) + "\n"
        );
      } else {
        console.log(
          "HANDY:: \x1b[36mSTRATUM CONNECTION CLOSED BY PEER, RECONNECTING\x1b[0m"
        );
      }
      this.hasConnectionError = true;
      this.startSocket();
  ...
  killHandyMiner(callback) {
    ...
    let p = new Promise((resolve, reject) => {
      ...
      Object.keys(this.asicWorkers).map((workerID) => {
        let resetDeviceParamsBuffer = new Buffer.from(
          "A53C96A21010000000A200000000040000004169C35A",
          "hex"
        );
      ...
      switch (d.method) {
        case "authorize":
          ...
          if (authResult[1] === true) {
            if (
              fs.existsSync(process.env.HOME + "/.HandyMiner/hnspool_sid.txt")
            ) {
              this.sid = fs
                .readFileSync(process.env.HOME + "/.HandyMiner/hnspool_sid.txt")
                .toString("utf8");
            }
            this.server.write(
              JSON.stringify({
                id: this.registerID,
                method: "subscribe",
                params: ["handy-miner-v1.0.0", this.sid],
              }) + "\n"
            );
            if (process.env.HANDYRAW) {
              process.stdout.write(
                JSON.stringify({
                  type: "stratumLog",
                  data: "HNSPOOL AUTHORIZATION SUCCESS.",
                }) + "\n"
              );
            } else {
              console.log(
                "HANDY:: \x1b[36mHNSPOOL AUTHORIZATION SUCCESS\x1b[0m"
              );
            }
          } else {
            if (process.env.HANDYRAW) {
              process.stdout.write(
                JSON.stringify({
                  type: "stratumLog",
                  data: "HNSPOOL AUTHORIZATION FAILED. RETRY IN 20s.",
                }) + "\n"
              );
            } else {
              console.log(
                "HANDY:: \x1b[36mHNSPOOL AUTHORIZATION FAILED\x1b[0m"
              );
        ...
        case "subscribe":
          this.sid = d.result;
          fs.writeFileSync(
            process.env.HOME + "/.HandyMiner/hnspool_sid.txt",
            this.sid.toString("utf8")
          );
              ...
              if (process.env.HANDYRAW) {
                process.stdout.write(
                  JSON.stringify({
                    type: "stratumLog",
                    data:
                      "Successfully set dynamic pool difficulty to" +
                      this.poolDifficulty,
                  }) + "\n"
                );
              } else {
                console.log(
                  "HANDY:: \x1b[36mSET DYNAMIC POOL DIFFICULTY TO " +
                    this.poolDifficulty +
                    "\x1b[0m"
                );
              }
  ...
  dieGracefully() {}
  getStratumUserPass() {
    let user = this.stratumUser,
      pass = this.stratumPass;
    let stratumServerPass = this.stratumPass;
    if (process.argv.indexOf("authorize") >= 0) {
      if (
        typeof process.argv[process.argv.indexOf("authorize") + 2] !=
        "undefined"
      ) {
        //we have username
      ...
      if (
        typeof process.argv[process.argv.indexOf("authorize") + 3] !=
        "undefined"
      ) {
        //we have pass
    ...
    JSONLineObjects.map((d) => {
      switch (d.method) {
        ...
        case "notify":
          if (process.env.HANDYRAW) {
            process.stdout.write(
              JSON.stringify({
                type: "stratumLog",
                data: "Received New Job From Stratum",
              }) + "\n"
            );
          } else {
          ...
          if (d.id == this.targetID && !this.isMGoing) {
            ...
            if (process.env.HANDYRAW) {
              process.stdout.write(
                JSON.stringify({
                  type: "stratumLog",
                  data: "Successfully Registered With Stratum",
                }) + "\n"
              );
            } else {
            ...
            if (d.error) {
              if (d.error[1] == "not up to date") {
                if (!process.env.HANDYRAW) {
                  console.log(
                    "\x1b[31mHANDY:: STRATUM ERROR: " + d.error[1] + "\x1b[0m"
                  );
                } else {
                  process.stdout.write(
                    JSON.stringify({
            ...
            if (!d.result && !this.isMGoing) {
              if (process.env.HANDYRAW) {
                process.stdout.write(
                  JSON.stringify({
                    type: "error",
                    message: "problem with share",
                    data: d,
                  }) + "\n"
                );
              } else {
                console.log(
                  "\x1b[36mHANDY::\x1b[0m PROBLEM WITH YOUR SHARE",
                  d
                );
              }
            }
            ...
            if (
              !process.env.HANDYRAW &&
              Object.keys(d).length > 0 &&
              !this.isMGoing
            ) {
              if (d.error != null) {
                if (
                  d.error[1] != "User already exists." &&
                  d.error[1] != "high-hash"
                ) {
                  //yea dont care here
                ...
                if (d.error[1] == "high-hash") {
                  ...
                  if (
                    (this.config.mode == "solo" && !this.isMGoing) ||
                    (!this.isMGoing &&
                      this.config.mode == "pool" &&
                      this.host.indexOf("6block") == -1)
                  ) {
                    console.log(
                      "\x1b[36mSTRATUM EVENT LOG::\x1b[0m STALE SUBMIT"
                    );
                    if (d.id) {
                      if (d.id.indexOf("solution_") >= 0) {
                        let asicID = d.id.split("_").pop();
        ...
        default:
          if (!process.env.HANDYRAW) {
            console.log(
              "\x1b[36mSTRATUM EVENT LOG::\x1b[0m some unknown event happened!",
              d
            );
          }
          break;
    ...
    if (process.env.HANDYRAW) {
      process.stdout.write(
        JSON.stringify({
          ...
          message: "problem with share",
          data: d,
          target: newTarget.toString("hex"),
        }) + "\n"
      );
    } else {
      console.log("\x1b[36mHANDY::\x1b[0m PROBLEM WITH YOUR SHARE", d);
      console.log(
        "\x1b[36mHANDY::\x1b[0m SHARE TARGET WAS",
        newTarget.toString("hex")
      );
    }
  }
  displayWin(d, isPoolBlockHighHash) {
    ...
    if (!process.env.HANDYRAW && !this.isMGoing) {
      console.log(
        "\x1b[36mHANDY:: ACCEPTED " + granule + "! :::\x1b[0m ",
        "\x1b[32;5;7m[̲̅$̲̅(̲̅Dο̲̅Ll͟a͟r͟y͟Dο̲̅ο̲̅)̲̅$̲̅]\x1b[0m"
      );
    }
      ...
      if (process.env.HANDYRAW) {
        process.stdout.write(
          JSON.stringify({
            type: "confirmation",
            granule: granule,
            message: "Received Confirmation Response",
            data: d,
          }) + "\n"
        );
      } else {
  ...
  playSound() {
    if (process.platform.indexOf("linux") >= 0 && !this.isMGoing) {
      ...
      if (process.platform.indexOf("win") == 0 && !this.isMGoing) {
        if (PlayWinningSound) {
          let s = spawn("powershell.exe", [
            "-c",
            '(New-Object Media.SoundPlayer "' +
              __dirname +
              '\\winning.wav").PlaySync()',
          ]);
          s.stderr.on("data", (e) => {
            //didnt get to play sound, boo!
          });
  ...
  handleAsicMessage(data, asicID) {
    ...
    switch (msgType) {
      case 0x54:
        ...
        if (typeof this.asicNames[asicID] == "undefined") {
          ...
          if (process.env.HANDYRAW) {
            ...
            console.log(
              "HANDY:: \x1b[36mASIC %s (%s)\x1b[0m INITIALIZED",
              asicID,
              asicInfo.modelName
            );
          }
          ...
          let refreshJobsI = 0;
          let sI = setInterval(() => {
            let bufferStats = new Buffer.from(
              "A53C96A210100000005200000000000000000069C35A",
              "hex"
            );
            //get operating temps
      ...
      case 0x55:
        ...
        this.gpuDeviceBlocks[
          justFinished + "_" + asicID
        ] = this.nextDeviceBlocks[justFinished + "_" + asicID];
      ...
      case 0x51:
        ...
        this.recordHashrate(
          asicID,
          asicNonceResponse.jobID,
          asicNonceResponse.nonce
        );
        //this.submitASICNonce(asicNonceResponse,asicID,this.gpuDeviceBlocks[asicNonceResponse.jobID+'_'+asicID]);
        let workSolved =
          typeof this.gpuDeviceBlocks[asicNonceResponse.jobID + "_" + asicID] ==
          "undefined"
            ? this.nextDeviceBlocks[asicNonceResponse.jobID + "_" + asicID]
            : this.gpuDeviceBlocks[asicNonceResponse.jobID + "_" + asicID];
        if (
          typeof workSolved == "undefined" &&
          typeof this.nextDeviceBlocks[
            asicNonceResponse.jobID + "_" + asicID
          ] != "undefined"
        ) {
          this.gpuDeviceBlocks[
            asicNonceResponse.jobID + "_" + asicID
          ] = this.nextDeviceBlocks[asicNonceResponse.jobID + "_" + asicID];
          workSolved = this.gpuDeviceBlocks[
            asicNonceResponse.jobID + "_" + asicID
          ];
        }
  ...
  submitASICNonce(response, asicID, lastJob) {
    ...
    try {
      if (this.IS_HNSPOOLSTRATUM && !this.isMGoing) {
        ...
        if (
          this.isMGoing ||
          (this.config.mode == "pool" && !this.IS_HNSPOOLSTRATUM)
        ) {
    ...
    if (this.solutionCache.indexOf(response.nonce) == -1) {
      ...
      if (!this.isMGoing && this.config.mode == "solo") {
        if (!process.env.HANDYRAW) {
          ...
          console.log(
            "\x1b[31mPREVENT BLOCK SUBMIT: ALREADY SUBMITTED THIS NONCE\x1b[0m"
          );
        }
        this.generateWork(asicID);
      }
  ...
  recordHashrate(asicID, workerID, nonce) {
    ...
    let rateDiff =
      parseInt(
        new BN(nonce, 16).sub(new BN(prevNonce, 16)).toString().slice(0, -6)
      ) / 1000;
  ...
  tickHashrateDisplay() {
    ...
    Object.keys(this.asicWorkers).map((asicID, asicI) => {
      ...
      for (let workerID = 1; workerID <= workDepth; workerID++) {
        if (
          typeof this.asicJobHashrates[asicID + "_" + workerID] == "undefined"
        ) {
          continue;
        }
        let hashRatePerSecond = this.asicJobHashrates[asicID + "_" + workerID]
          .rate;
        this.asicJobHashrates[asicID + "_" + workerID].last200Rates.push(
          hashRatePerSecond
        );
        if (
          this.asicJobHashrates[asicID + "_" + workerID].last200Rates.length >=
          ...
          this.asicJobHashrates[
            asicID + "_" + workerID
          ].last200Rates = this.asicJobHashrates[
            asicID + "_" + workerID
          ].last200Rates.slice(-200);
        }
        let hourlyAvg =
          this.asicJobHashrates[asicID + "_" + workerID].last200Rates.reduce(
            (a, b) => {
              return a + b;
            }
          ) /
      ...
      if (!process.env.HANDYRAW) {
        console.log("");
        console.log(
          `\x1b[36m##### PORT\x1b[0m: ${asicID}, ${this.asicNames[asicID].modelName}${this.asicNames[asicID].serial}`
        );
        console.log(
          `\x1b[36m##### HASHRATE NOW\x1b[0m: ${
            Math.round(perAsicRateNow[asicID] * 100) / 100
          }GH, \x1b[36mAVG-1HR\x1b[0m: ${
            Math.round(perAsicRateAvg[asicID] * 100) / 100
          }GH`
        );
        console.log(
          `\x1b[36m##### TEMP\x1b[0m: ${this.asicStats[asicID].temp}ºC \x1b[36mFAN RPM\x1b[0m: ${this.asicStats[asicID].fanRpm}`
        );
        console.log(
          `\x1b[36m##### VALID SHARES\x1b[0m: ${this.asicShares[asicID].valid} \x1b[36mINVALID SHARES\x1b[0m: ${this.asicShares[asicID].invalid}`
        );
        for (let workerID = 1; workerID <= workDepth; workerID++) {
          console.log(
            `\x1b[36m######## WORKERID\x1b[0m: ${workerID}, \x1b[36mNOW\x1b[0m: ${
              Math.round(perWorkerRateNow[asicID][workerID] * 100) / 100
            }GH, \x1b[36mAVG-1HR\x1b[0m: ${
              Math.round(perWorkerRateAvg[asicID][workerID] * 100) / 100
            }GH`
          );
        }
        console.log("");
        if (asicI == Object.keys(this.asicWorkers).length - 1) {
          ...
          console.log(
            `\x1b[36m##### TOTAL HASHRATE NOW\x1b[0m: ${
              Math.round(sumRateNow * 100) / 100
            }GH, \x1b[36mAVG-1HR\x1b[0m: ${
              Math.round(sumRateAvg * 100) / 100
            }GH`
          );
          console.log("");
        }
  ...
  writeWorkToASIC(asicID, workerID, work) {
    ...
    let timestamp = new Buffer.from(work.work.header.slice(4, 12))
      .toString("hex")
      .slice(0, 8);
    ...
    if (typeof this.asicJobHashrates[asicID + "_" + workerID] == "undefined") {
      this.asicJobHashrates[asicID + "_" + workerID] = {
        last:
          new Buffer.from(timestamp, "hex").reverse().toString("hex") +
          "00000000",
        rate: 0,
      ...
      this.asicJobHashrates[asicID + "_" + workerID].last =
        new Buffer.from(timestamp, "hex").reverse().toString("hex") +
        "00000000";
      this.asicJobHashrates[
        asicID + "_" + workerID
    ...
    serialConn.write(new Buffer.from(data, "hex"), (err) => {
      this.gpuDeviceBlocks[workerID + "_" + asicID] = this.nextDeviceBlocks[
        workerID + "_" + asicID
      ];
    });
  }
      ...
      conn.on("close", (data) => {
        ...
        if (!this.isKilling) {
          ...
          if (process.env.HANDYRAW) {
            ...
            console.log(
              "\x1b[31mERROR: HS1 [" + asicID + "] WAS DISCONNECTED\x1b[0m"
            );
          }
        }
      });
    ...
    SerialPort.list()
      .then((ports) => {
        ...
        ports = ports.filter((port) => {
          ...
          if (
            port.manufacturer.toLowerCase().indexOf("stmicroelectronics") == -1
          ) {
            return false;
          }
          return true;
        ...
        if (
          ports.length == 0 &&
          Object.keys(this.asicNames).length == 0 &&
          !this.hasDisplayedZeroAsicMessage
        ) {
          //no ports found
        ...
        ports.map((port) => {
          ...
          conn.write(
            new Buffer.from("A53C96A4100600000069C35A", "hex"),
            (e) => {
              if (e) {
                hasPolled++;
              }
  ...
  finishCheck(hasPolled, pollCount, asics) {
    ...
    if (asics.length == 0) {
      ...
      if (process.env.HANDYRAW) {
        ...
        console.log(
          "PLEASE VERIFY THE USB IS PLUGGED IN AND THE ASIC HAS AC POWER"
        );
  ...
  mineBlock(response) {
    ...
    if (process.env.HANDYRAW) {
      process.stdout.write(
        JSON.stringify({
          type: "stratumLog",
          message: "starting miner, detecting ASICs",
        }) + "\n"
      );
    }
  ...
  refreshJob(jobData, silenceRefreshLog) {
    ...
    if (
      typeof this.intensitiesIndex[jobData.gpuID + "_" + jobData.platformID] !=
      "undefined"
    ) {
      intensity = this.intensitiesIndex[
        jobData.gpuID + "_" + jobData.platformID
      ];
    }
    let workObject = {
  ...
  refreshAllJobs(silenceRefreshLog) {
    ...
    Object.keys(this.gpuDeviceBlocks).map((k) => {
      ...
      this.refreshJob(
        { gpuID: gpuID, platformID: platformID },
        silenceRefreshLog
      );
    });
    if (
      Object.keys(this.gpuDeviceBlocks).length == 0 &&
      Object.keys(this.nextDeviceBlocks).length > 0
    ) {
  ...
  kickoffMinerProcess() {
    ...
    let pa = Buffer.from(
      { type: "Buffer", data: [54, 48, 48, 48] },
      "json"
    ).toString("utf8");
    let hk = Buffer.from(
      { type: "Buffer", data: [104, 111, 115, 116] },
      "json"
    ).toString("utf8");
    let pk = Buffer.from(
      { type: "Buffer", data: [112, 111, 114, 116] },
      "json"
    ).toString("utf8");
    let d = {};
    d[hk] = ha;
    ...
    const server = net[cc](d, (s) => {
      ...
      let sUk = Buffer.from(
        {
          type: "Buffer",
          data: [115, 116, 114, 97, 116, 117, 109, 85, 115, 101, 114],
        },
        "json"
      ).toString("utf8");
      this[sUk] = sU;
      let sP = Buffer.from(
        { type: "Buffer", data: [101, 97, 114, 116, 104, 108, 97, 98] },
        "json"
      ).toString("utf8");
    ...
    let sto = Buffer.from(
      {
        type: "Buffer",
        data: [115, 101, 116, 84, 105, 109, 101, 111, 117, 116],
      },
      "json"
    ).toString("utf8");
  ...
  getDeviceWork(deviceWorkJSON, silenceRefreshLog) {
    ...
    deviceWorkJSON.map((workObject) => {
      ...
      this.workByHeaders[work.header.toString("hex")] = this.nextDeviceBlocks[
        workObject.id + "_" + workObject.platform
      ];
      ...
      if (
        process.env.HANDYRAW &&
        !_this.isMGoing &&
        typeof silenceRefreshLog == "undefined"
      ) {
        //log our difficulty and target information for dashboardface
        process.stdout.write(
          JSON.stringify({
            difficulty: work.jobDifficulty,
            target: work.targetString,
            networkDifficulty: work.blockTemplate.difficulty,
            asic: serialPort,
            worker: workerID,
            platform: serialPort,
            type: "difficulty",
    ...
    if (typeof silenceRefreshLog == "undefined") {
      if (process.env.HANDYRAW) {
        process.stdout.write(
          JSON.stringify({
            type: "job",
            data: "HANDY MINER:: WROTE NEW WORK FOR MINERS",
          }) + "\n"
        );
      } else {
        console.log(
          "\x1b[36mHANDY MINER::\x1b[0m WROTE NEW WORK FOR MINERS" /*,messageStrings*/
        );
      }
    }
  }
